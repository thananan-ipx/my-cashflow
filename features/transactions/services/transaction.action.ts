...
export async function addExpense(data: {
  ...
  if (
    data.debtId &&
    data.currentRemainingInstallments !== null &&
    data.currentRemainingInstallments !== undefined &&
    data.monthlyPayment
  ) {
    const installmentsToDeduct = Math.max(
      1,
      Math.round(data.amount / data.monthlyPayment)
    )
    const newRemaining = Math.max(
      0,
      data.currentRemainingInstallments - installmentsToDeduct
    )
